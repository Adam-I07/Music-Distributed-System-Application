import socket
import threading
from authenticationConnectionHandler import authenticationConnectionHandler
...
class authenticationNetworkInterface:
    def __init__(self):
        self.listeners = []
        self.connectionHandler = authenticationConnectionHandler()
        self.running = True
    ...
    def start_authentication(self, ip, port, duration=20, retries=30):
        ...
        while not connected and attempts < retries:
            try:
                print("Trying to connect to the bootstrap.")
