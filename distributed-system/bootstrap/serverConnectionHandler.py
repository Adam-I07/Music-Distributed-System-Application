...
class Connection:
    ...
    def add_connection_node(self, name, ip, port):
        try:
            if os.path.exists('connected_nodes_information.json'):
                with open('connected_nodes_information.json', 'r') as f:
                    data = json.load(f)
            else:
                data = {"connections": []}

            exists = any(entry["connection_type"] == name and entry["ip"] == ip and entry["port"] == port for entry in data["connections"])
            ...
            if not exists:
                ...
                with open('connected_nodes_information.json', 'w') as f:
