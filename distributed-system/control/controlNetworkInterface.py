import socket
import threading
from controlConnectionHandler import controlConnectionHandler
...
class controlNetworkInterface:
    def __init__(self):
        self.listeners = []
        self.connectionHandler = controlConnectionHandler()
        self.running = True
    ...
    def start_control(self, ip, port, duration=20, retries=30):
        ...
        # Attempting to establish connection with bootstrap, will keep retrying
        while not connected and attempts < retries:
            try:
                print("Trying to connect to bootstrap")
